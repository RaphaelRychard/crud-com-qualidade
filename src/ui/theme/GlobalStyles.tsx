      ...
      {`
        :root {
          ...
          ${Object.keys(theme.neutral)
            .map((key) => `--color-primary-${key}: ${theme.primary[key as keyof typeof theme.primary]};`)
            .join("")}
          ${Object.keys(theme.neutral)
            .map((key) => `--color-neutral-${key}: ${theme.neutral[key as keyof typeof theme.neutral]};`)
            .join("")}
          ${Object.keys(theme.negative)
            .map((key) => `--color-negative-${key}: ${theme.negative[key as keyof typeof theme.negative]};`)
            .join("")}
        }
